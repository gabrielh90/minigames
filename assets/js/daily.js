...
(function(){
  ...
  function isAvailable(lastISO){ return lastISO !== todayISO(); }
  function claim(){
    const reward = (window.CONFIG?.DAILY_REWARD?.candies ?? 3);
    const st = window.State.get();
    if(!isAvailable(st.lastDailyReward)) return { ok:false, error: 'Ai luat deja recompensa azi.' };
    window.State.addCandies(reward); window.State.setLastDailyReward(todayISO());
