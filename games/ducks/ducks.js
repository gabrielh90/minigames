...
// Nu păstrează scor global local; raportează câștigurile cu postMessage.
(function () {
  ...
  // Poți citi starea la start dacă vrei avatar/equipment
  ...
  window.addEventListener("message", (e) => {
    ...
    if (type === "playerState") {
      // adaptează UI la avatar dacă e cazul
    }
    if (type === "avatarChanged") {
      // schimbă sprite-ul dacă e cazul
    }
  });
  ...
  function Timer(seconds, onTick, onEnd) {
    let remain = seconds;
    const id = setInterval(() => { remain--; onTick?.(remain); if (remain <= 0) { clearInterval(id); onEnd?.(); } }, 1000);
    onTick?.(remain);
  ...
  function startGame() {
    ...
    function computeCirclePositions(count = 10) {
      const { w, h } = pondRect(); const c = center();
      const margin = 24;
      const radius = Math.max(120, Math.min(w, h)/2 - padSize/2 - margin);
      const list = [];
      for (let i=0;i<count;i++){
        const angle = (i / count) * Math.PI * 2 - Math.PI / 2; // începe sus
        list.push({ x: Math.round(c.x + radius * Math.cos(angle)), y: Math.round(c.y + radius * Math.sin(angle)) });
      }
      return list;
    }
    function moveDuckTo(x, y, bounce=true){
      duck.style.left = `${x}px`; duck.style.top = `${y}px`;
      if (bounce) { duck.classList.remove("jump"); void duck.offsetHeight; duck.classList.add("jump"); }
      const splash = document.createElement("div"); splash.className = "splash";
      splash.style.left = `${x - 6}px`; splash.style.top = `${y - 6}px`; pond.appendChild(splash);
      setTimeout(()=>splash.remove(), 600);
    }
    function padCenter(pad){ return { x: parseInt(pad.style.left,10) + padSize/2, y: parseInt(pad.style.top,10) + padSize/2 }; }
    ...
    function endWithFall(pad){
      ...
      setTimeout(()=>{ pad.classList.add("fall"); duck.classList.add("fall"); setTimeout(()=>showEndScreen("fail"), 900); }, 150);
    }

    // 'fail' = greșeală; 'timeout' = timp expirat; 'summary' = runda încheiată cu scor > 0
    function showEndScreen(mode){
      ...
      let title = "Runda încheiată";
      if (mode === "fail") title = "Ai pierdut";
      if (mode === "timeout") title = "Timp expirat";
    ...
    // rața începe în centru
    const c = center(); moveDuckTo(c.x, c.y, false);

    // timer: la expirare NU mai afișăm „Felicitări!” implicit
    const tmr = Timer(45, (r)=>{ const el = document.getElementById("duckTime"); if (el) el.textContent = `⏳ ${r}`; }, ()=>{
      if (ended) return;
      ended = true;
      // dacă ai cel puțin 1 corect, e un sumar; altfel e „Timp expirat”
      showEndScreen(correct > 0 ? "summary" : "timeout");
    });
    ...
    window.addEventListener("resize", () => {
      ...
      resizeId = setTimeout(() => {
        ...
        if (currentDuckPad) {
          ...
          if (pad) { const { x, y } = padCenter(pad); moveDuckTo(x, y, false); }
        } else {
          const c = center(); moveDuckTo(c.x, c.y, false);
        }
      }, 100);
